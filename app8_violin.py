...
# pour le calendrier
def get_death_by_day(year, laconn):
    ...
    # print("ICI")
    query = f"""
    SELECT DATE(dd_an || '-' || printf('%02d', dd_mois) || '-' || printf('%02d', dd_jour)) AS date, sexe,COUNT(*) as Nbre
    FROM personnes
    where dd_an='{year}'
     GROUP BY dd_mois,dd_jour,sexe
    """
    df = pd.read_sql_query(query, laconn)
    ...
    # Calculer le total des deces par mois
    totals = df.groupby('date')['Nbre'].sum().reset_index()

    # Ajouter une colonne 'sexe' pour marquer les lignes de total
    totals['sexe'] = 'Total'

    # Fusionner les totaux avec le DataFrame original
    df = pd.concat([df, totals[['date', 'sexe', 'Nbre']]], ignore_index=True)

    # Trier le DataFrame par 'dd_mois' pour maintenir l'ordre
    df = df.sort_values(by=['date', 'sexe'], ascending=[True, False]).reset_index(drop=True)
...
def update_graph(selected_year,selected_sexe):
    ...
    # Datas pour les boites a moustaches
    #df_boite_moustache = get_age_by_year_sexe(selected_year, conn)

    # Datas pour les violons
    #df_age = df_boite_moustache.copy()
    df_age = get_age_by_year_sexe(selected_year, conn)
    ...
    # Datas pour la carte calendrier des décès et convertion en DataFrame Pandas
    df_calendar = pd.DataFrame(get_death_by_day(selected_year, conn))
    ...
    #df_birth_year_current=df_birth[df_birth[''] == selected_year]
    ...
    # Ajouter les données pour les hommes
    fig_violin = go.Figure()


    # Afficher les trois violons
    ...
    if selected_sexe != 'Total':
        filtered_df = df_calendar[df_calendar['sexe'] == selected_sexe].copy()

    # Définir la couleur
    selected_color = color_map_choro[selected_sexe]

    # Créer le calendrier avec plotly_calplot
    fig_calendar = calplot.calplot(
        filtered_df,
        x='date',  # Colonne avec les dates
        y='Nbre',  # Colonne avec les valeurs de décès
    ...
    #return f"Total décès: {total_enreg_death}", f"Décès hommes: {hommes_death}", f"Décès femmes: {femmes_death}",f"Total Naissances: {total_enreg_birth}", f"Naissances Garçons: {hommes_birth}", f"Naissances Filles: {femmes_birth}", fig_moustache, fig_birth, fig_death_mois, fig_death_year, fig_violin,fig_choro_birth, fig_choro_death, fig_calendar
    return f"Total décès: {total_enreg_death}", f"Décès hommes: {hommes_death}", f"Décès femmes: {femmes_death}",f"Total Naissances: {total_enreg_birth}", f"Naissances Garçons: {hommes_birth}", f"Naissances Filles: {femmes_birth}", fig_birth, fig_death_mois, fig_death_year, fig_violin,fig_choro_birth, fig_choro_death, fig_calendar
