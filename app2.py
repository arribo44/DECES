...
def calculate_indicators(df):
    ...
    hommes = df[df['sexe'] == 1]['Nbre'].values[0]  # Total des hommes
    femmes = df[df['sexe'] == 2]['Nbre'].values[0]  # Total des femmes
...
def update_graph(selected_year):
    ...
    fig_death_year.update_layout(
        ...
        xaxis_title="Annee",
        yaxis_title="Nombre de naissance",
        #xaxis=dict(tickmode='array', tickvals=df_death_year['dd_an'].unique(), ticktext=[
        #    'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']),
        template="plotly_white"
    )

    return fig_birth, fig_death_mois, fig_death_year
